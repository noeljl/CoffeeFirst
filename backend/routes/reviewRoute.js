...
reviewRouter.get(
  '/coffee-shop/:coffeeShopId/summary',
  async (req, res, next) => {
    try {
      const reviews = await reviewService.getReviewsByCoffeeShopId(
        req.params.coffeeShopId
      ...
      const mode = (arr) => {
        ...
        if (
          ...
          uniqueValues.every((val) => val === 'true' || val === 'false')
        ) {
      ...
      const presentationArr = reviews
        .map((r) => r.presentation)
        .filter(Number.isFinite)
      const temperatureArr = reviews
        .map((r) => r.temperature)
        .filter((val) => val === true || val === false)
      const vibeArr = reviews
        .map((r) => r.vibe)
        .filter((val) => val !== null && val !== undefined)
      const aestheticsArr = reviews
        .map((r) => r.aesthetics)
        .filter((val) => val === true || val === false)
      const serviceFriendlinessArr = reviews
        .map((r) => r.serviceFriendliness)
        .filter(Number.isFinite)
      const pricingArr = reviews
        .map((r) => r.pricing)
        .filter((val) => val !== null && val !== undefined)
      const ecoFriendlyArr = reviews
        .map((r) => r.ecoFriendly)
        .filter((val) => val === true || val === false)
      const veganFriendlyArr = reviews
        .map((r) => r.veganFriendly)
        .filter((val) => val === true || val === false)
      const instagramArr = reviews
        .map((r) => r.instagram)
        .filter((val) => val === true || val === false)
      const greatForStudyingArr = reviews
        .map((r) => r.greatForStudying)
        .filter((val) => val === true || val === false)
      const dateSpotArr = reviews
        .map((r) => r.dateSpot)
        .filter((val) => val === true || val === false)
      const petFriendlyArr = reviews
        .map((r) => r.petFriendly)
        .filter((val) => val === true || val === false)
